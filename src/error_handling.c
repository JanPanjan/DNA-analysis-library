...
const ResultInfo result_messages[] = {
        ...
        {ERROR_INVALID_USAGE,            "ERROR_INVALID_USAGE: Correct usage is 'dna-processing-lib <dna_file>'."},
        {READER_FAIL,                    "READER_FAIL: Error reading the file."},
        {DNA_ANALYSIS_INVALID_CHARACTER, "DNA_ANALYSIS_INVALID_CHARACTER: Invalid character in DNA sequence."},
        {DNA_ANALYSIS_NULL_SEQUENCE,     "DNA_ANALYSIS_NULL_SEQUENCE: Invalid DNA sequence."},
...
void get_result(const Result result) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if ((int) result.result_code == rc) {
            fprintf(
                    stderr,
                    "Error %d occurred in %s at line %d:\n  %s %s\n",
                    rc, result.file, result.linenr, rm, result.context
            );
            exit(rc);
        }
    }
    ...
    fprintf(
            stderr,
            "Error '%s' %d in %s at line %d\n",
            "Unknown error code", result.result_code, result.file, result.linenr
    );
