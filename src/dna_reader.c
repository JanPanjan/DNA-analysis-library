 ...
 * struct. If any of these operations fail, it returns the corresponding error
 * code.
 */
Result read_dna_sequence(const char *filename, Sequence *sequence_info) {
    ...
    if (file == NULL) {
        return (Result) {
                ...
                __LINE__,
                __FILE__,
                "Can't read file, because is it null. Maybe filename is spelled wrong?"
        };
    }

    // Allocate memory for sequence
    Result seq_allocation_result;
    sequence_info->raw = _allocate_memory_(MAX_SEQUENCE_LENGTH, &seq_allocation_result);

    if (seq_allocation_result.result_code != RESULT_SUCCESS) {
        return (Result) {
                seq_allocation_result.result_code,
                __LINE__,
                __FILE__,
